    ...
    for( let spherePost of AllSpherePostInDatabase){
        ...
        if(spherePost.UserLikedBy.length > 0){
            for( let i = 0; i < spherePost.UserLikedBy.length; i++){
                ...
                if(spherePost.UserLikedBy[i]  === oldNickname){
                    ...
                    spherePost.UserLikedBy.splice(i, 1)
        ...
        if(spherePost.Comments.length > 0){
            ...
            for(let i = 0; i < spherePost.Comments.length; i++){
                // console.log(spherePost.Comments[i][0].length, oldNickname.length,  spherePost.Comments[i][0] === oldNickname)
...
const patchSharedPosts = async (req, res) =>{
    const {shared} = req.body
    // console.log(shared)
    const displaySharedPosts = []
    try{
        const db = await connectToDatabase();
        const AllSpherePostInDatabase = await db.db("NewsFeed").collection("SpherePost").find().toArray();
        for(let spherePost of AllSpherePostInDatabase){
            // console.log(shared._id, spherePost._id, shared._id === spherePost._id)
            for(let userShared of shared){
                if(userShared._id === spherePost._id){
                    displaySharedPosts.push(spherePost)
                }
            }
        }
        res.status(200).json({status: 200, message:"update is complete", displaySharedPost: displaySharedPosts})
    }catch (err) {
        res.status(404).json({ status: 404, message: err.message });
    }
}
